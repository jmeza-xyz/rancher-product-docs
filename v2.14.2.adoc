...
:previous-release-version: v2.14.1
...
ifeval::["{build-type}" != "community"]
Rancher v2.14.2 is the latest patch release of Rancher. This is a Prime version release that introduces maintenance updates and bug fixes. To learn more about Rancher Prime, see our page on the https://www.rancher.com/products/rancher-platform[Rancher Prime Platform].
endif::[]
...
* Fixed a security vulnerability in the GitHub App authentication provider where users incorrectly inherited permissions from all teams within their GitHub organization, rather than only the specific teams to which they belonged. Upon upgrading, Rancher automatically triggers a mandatory refresh of all affected user `principals` to remove these incorrectly assigned team memberships and restore proper access. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-4j6x-2764-m8gh[CVE-2026-41053].
* Updated the permissions of the built-in `project-owner` role to no longer include the `updatepsa` verb. This prevents users with this role from bypassing restricted PSA policies or deploying privileged workloads within their projects. If your organization requires users to retain this capability, administrators must create a custom project role that explicitly grants the `updatepsa` verb for the project resource. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-vx8h-4prv-g744[CVE-2026-41052].
* Rancher now validates the `authImage` parameter in cluster import manifests to prevent YAML injection attacks. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-mhc6-2gfq-xx62[CVE-2026-44939].

[#_fleet]
=== Fleet

* Resolved a conceptual security issue for multi-tenant and multi-user single-cluster environments by introducing a `Policy` resource. If you have been using `GitRepoRestriction`, please migrate to `Policy`. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-xr65-5cpm-g36x[CVE-2026-44935].
* Fleet now mandates that `helmRepoURLRegex` be set on the `GitRepo` resource before Helm repository credentials are forwarded to chart repository URLs specified in `fleet.yaml`. Existing `GitRepo` resources that have a Helm secret configured are automatically migrated upon upgrade to preserve their previous behavior. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-hx4v-cxpf-vh8m[CVE-2026-44936].
* Fixed a security vulnerability in the webhook component that enabled attackers to forge webhooks, which Fleet would then process if webhook shared secrets were not configured. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-jmf4-m7j9-g72r[CVE-2026-44937].
* Fleet now prevents `namespaceLabels` in `fleet.yaml` from overwriting Pod Security Standards enforcement labels on existing namespaces. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-864g-863m-vcvq[CVE-2026-44938].

[#_suse_observability]
=== SUSE Observability

* Fixed a security vulnerability that exposed service tokens by migrating the token to a secure Kubernetes Secret that is automatically deleted when the extension is uninstalled. For more information refer to https://github.com/StackVista/rancher-extension-stackstate/security/advisories/GHSA-7c27-jc6w-pw95[CVE-2026-44940].

[#_suse_rancher_ai_assistant]
=== SUSE Rancher AI Assistant

* Fixed an information disclosure vulnerability where sensitive configuration secrets and MCP responses were exposed when the logging level was set to `DEBUG. For more information, refer to https://github.com/rancher/rancher-ai-agent/security/advisories/GHSA-5r2r-h824-fr5v[CVE-2026-44934].
...
* A known issue can cause Rancher Logging DaemonSets (`rancher-logging-rke2-journald-aggregator` and `rancher-logging-root-fluentbit`) to enter `CrashLoopBackOff` or remain unstable when host `inotify` limits are too low. As a workaround, increase `fs.inotify.max_user_watches` and `fs.inotify.max_user_instances` on affected nodes. See https://github.com/rancher/rancher/issues/54216[#54216].
...
* It is now possible to install the same chart multiple times in the same cluster from *Apps* > *Charts* by using *Install as a new instance*. See https://github.com/rancher/dashboard/issues/16927[#16927] and https://github.com/rancher/dashboard/issues/16907[#16907].
* Fixed an issue where add-on chart values could be lost when viewing or editing cluster configuration in the Rancher UI. See https://github.com/rancher/dashboard/issues/17527[#17527] and https://github.com/rancher/dashboard/issues/17498[#17498].
