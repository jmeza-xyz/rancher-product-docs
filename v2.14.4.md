...
Rancher v2.14.4 is the latest patch release of Rancher. This is a Prime version release that introduces maintenance updates and bug fixes. To learn more about Rancher Prime, see our page on the [Rancher Prime Platform](https://www.rancher.com/products/rancher-platform).
...
- Rancher now protects against Denial of Service attacks on login endpoints by only reading request bodies up to 1MiB. Users with admin permissions can alter this limit by changing the setting `public-api-body-limit` by updating the corresponding global setting resource through the v3 API or via kubectl. For more information, refer to [CVE-2026-59675](https://github.com/rancher/rancher/security/advisories/GHSA-g4f6-44g4-23xm).

- Fixed a security vulnerability where long-lived Rancher registration tokens were stored or exposed in plaintext, allowing an attacker who obtained a token to register a rogue node and potentially take over a downstream cluster. `ClusterRegistrationToken` credentials are now stored in Kubernetes Secrets with a TTL token rotation and expiration feature (opt-in in Rancher 2.14, enabled by default in Rancher 2.15 and later). The v2prov install script’s file permissions are hardened to owner-readable only for provisioning new machines. For more information, refer to [CVE-2026-55997](https://github.com/rancher/rancher/security/advisories/GHSA-7r53-jvhg-9jq4).

## Rancher General

### Major Bug Fixes

- Fixed an issue where some downstream clusters could remain in an `Unavailable` state after a Rancher pod restart even though their cluster agents were still connected. Rancher v2.14.4 now retries the affected controller reconciliation so the clusters recover automatically. See [\#55232](https://github.com/rancher/rancher/issues/55232).

## Changes Since v2.14.3

See the full list of [changes](https://github.com/rancher/rancher/compare/v2.14.3%E2%80%A6v2.14.4).
...
- rancher/rancher:v2.14.4

### Tools

- CLI - [v2.14.4](https://github.com/rancher/cli/releases/tag/v2.14.4)
