...
Rancher v2.14.5 is the latest patch release of Rancher. This is a Prime version release that introduces maintenance updates and bug fixes. To learn more about Rancher Prime, see our page on the [Rancher Prime Platform](https://www.rancher.com/products/rancher-platform).
...
See the full list of [changes](https://github.com/rancher/rancher/compare/v2.14.4%E2%80%A6v2.14.5).
...
>
>   - Refer to the {rn-aggregation-layer-is-required}\[Extension API Server documentation\] and [\#50400](https://github.com/rancher/rancher/issues/50400) for more information.
>
...
- **Requirements for air-gapped environments:**
  ...
  - When using a proxy in front of an air-gapped Rancher instance, you must pass additional parameters to `NO_PROXY`. See the {rn-install-rancher}\[documentation\] and issue [\#2725](https://github.com/rancher/docs/issues/2725#issuecomment-702454584).
...
- rancher/rancher:v2.14.5

### Tools

- CLI - [v2.14.5](https://github.com/rancher/cli/releases/tag/v2.14.5)
...
- **Rancher v2.14.0:**
  ...
  - When performing a rollback from Rancher v2.14 to Rancher v2.13 using the Backup/Restore Operator, the restore process requires all Rancher-related resources to be cleaned up on the upstream (local) cluster. For more information on the required rollback steps, refer to the {rn-rollback}\[Rancher documentation\]. See also the above release note on Cluster API **v1beta2**, introduced by the **cluster-api v1.12.2** upgrade, and [\#916](https://github.com/rancher/backup-restore-operator/issues/916).

### Previous Rancher Behavior Changes - Cluster Provisioning

- **Rancher v2.14.1:**

  - **Deprecation:** Starting from Rancher v2.14.1, Cluster API Addon Provider Fleet (CAAPF) is being disabled by default and will be removed in a future Rancher release per our [deprecation policy](https://www.suse.com/support/rancher-prime/#Rancher-Prime-Deprecation-Policy). An automated migration script for CAAPF resources is available when upgrading to Rancher v2.14.1, refer to the {rn-caapf-migration}\[CAPI documentation\]. While standard Fleet integration is still available through Rancher, additional steps are needed by users that upgrade to Rancher v2.14.1 and are running CAPI clusters that are provisioned using CAAPF. If you are upgrading CAPI clusters provisioned with CAAPF, keep `features.use-caapf.enabled=true` before upgrading to avoid Fleet-management handoff conflicts that can disrupt downstream workloads. Refer to {rn-capi-feature}\[Features\] and the {rn-caapf}\[Cluster API Addon Provider Fleet\] documentation for more information. See [\#54740](https://github.com/rancher/rancher/issues/54740).
...
- **Rancher v2.14.0:**
  ...
  - **Embedded Cluster API removed**: The built-in Rancher Provisioning Cluster API functionality (`rancher-provisioning-capi`) has been removed in Rancher v2.14.0, as Rancher Turtles is now the default mechanism for deploying CAPI CRDs and controllers. This includes:
    ...
    - **Migration Path:** The migration to Rancher Turtles happens automatically during the upgrade to v2.14.0. Rancher Turtles is now the only supported method for Cluster API (CAPI) integration with Rancher. Unless you are using a certificated CAPI provider installed through Turtles, no manual action is required.
      ...
      For more information on installing and using Rancher Turtles, refer to the {rn-capi-overview}\[Cluster API integration documentation\]. See [\#53291](https://github.com/rancher/rancher/issues/53291).
...
- **Rancher v2.14.0:**
  ...
  - **Deprecation** The `v1alpha1` API used in Cluster API Provider RKE2 (CAPRKE2) is deprecated. See [\#797](https://github.com/rancher/cluster-api-provider-rke2/issues/797).
