...
:release-version: v2.13.6
:rn-component-version: v2.13
:previous-release-version: v2.13.5
...
* Fixed an issue where add-on charts' values were not preserved when viewing or editing clusters in the Rancher UI. Using the *Edit Config* option could reset chart values to defaults. Editing cluster configurations using *Edit as YAML* directly or `kubectl` is not affected. See https://github.com/rancher/dashboard/issues/17526[#17526].
* Fixed an issue where there was no option to install a new instance of a chart that was already installed. It is now possible to install the same chart multiple times in the same cluster. See https://github.com/rancher/dashboard/issues/16926[#16926].
